...
public class TimeoutCommand extends SlashCommand {
    ...
    private static final FlatFileSection commands = Configuration.getCommands().getSection("timeout-command");
    ...
    public void onExecute(SlashExecutor cmd) {
        ...
        OptionMapping time = cmd.getOption("time");
        Integer parsedTime = time == null ? null : Utils.parseShortTime(time.getAsString());
        new PunishmentCreator(PunishmentType.MUTE, parsedTime == null || parsedTime == -1 ? null : (long) parsedTime, cmd.getOption("reason") == null ? null : cmd.getOption("reason").getAsString())
                .create(member.getIdLong(), cmd, false, ban -> {
                    new ActionLog(member, cmd.getMember(), "● Action: **Member Muted**\n● Reason: **" + (ban.getReason() == null ? "N/A" : ban.getReason()) + "**\n Time: **" + (ban.getTimeFor() == null ? "Forever" : Utils.formatTime(ban.getTimeFor())) + "**", Color.ORANGE);
                    SpotMessage spotMessage = new SpotMessage(Configuration.getLang().getSection("lang.punishment-created" + (ban.getTimeFor() == null ? "" : "-time") + (ban.getReason() == null ? "" : "-reason")), cmd);
                    spotMessage.replaceOptions("%member%", member.getAsMention());
                    spotMessage.replaceOptions("%punishment%", ban.getPunishmentType().getOtherName().toLowerCase());
                    spotMessage.replaceOptions("%reason%", ban.getReason() == null ? "N/A" : ban.getReason());
                    spotMessage.replaceOptions("%time%", ban.getTime() == null ? "N/A" : Utils.formatTime(ban.getTimeFor()));
