...
public class BanCommand extends SlashCommand {
    ...
    private static final FlatFileSection commands = Configuration.getCommands().getSection("ban-command");
    ...
    public void onExecute(SlashExecutor cmd) {
        ...
        new PunishmentCreator(PunishmentType.BAN, parsedTime == null || parsedTime == -1 ? null : (long) parsedTime, cmd.getOption("reason") == null ? null : cmd.getOption("reason").getAsString())
                .create(member.getIdLong(), cmd, false, ban -> {
                    new ActionLog(member, cmd.getMember(), "● Action: **Member Banned**\n● Reason: **" + (ban.getReason() == null ? "N/A" : ban.getReason()) + "**\n Time: **" + (ban.getTimeFor() == null ? "Forever" : Utils.formatTime(ban.getTimeFor())) + "**", Color.ORANGE);
