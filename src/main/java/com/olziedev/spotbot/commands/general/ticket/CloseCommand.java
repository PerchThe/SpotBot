...
public class CloseCommand extends SlashCommand {
    ...
    private static final FlatFileSection commands = Configuration.getCommands().getSection("close-command");
    ...
    public void onExecute(SlashExecutor cmd) {
        ...
        SpotMessage confirmMessage = new SpotMessage(Configuration.getConfig().getSection("settings.ticket.categories." + category.getKey() + ".ticket-confirmation-message"), cmd);
        if (confirmMessage.hasEmbed()) {
            confirmMessage.getSpotEmbed().replaceOptions("%member%", "<@" + ticket.getUserID() + ">");
        }
        confirmMessage.sendMessage().queue();
    }
}
