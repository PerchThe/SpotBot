...
public class DenyCommand extends SlashCommand {

    private static final FlatFileSection commands = Configuration.getCommands().getSection("deny-command");
    ...
    public void onExecute(SlashExecutor cmd) {
        ...
        suggestion.deny(cmd.getOption("reason").getAsString(), cmd.getMember(), msg -> {
            SpotMessage spotMessage = new SpotMessage(Configuration.getLang().getSection("lang.suggestion-rejected"), cmd);
            spotMessage.replaceOptions("%link%", String.format("https://discord.com/channels/%s/%s/%s", cmd.getGuild().getId(), Configuration.getConfig().getLong("settings.suggestion.denied-channel-id"), msg));
